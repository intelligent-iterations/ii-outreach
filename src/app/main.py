...
from src.runtime.models import ActionResult, ActionType, ClaimStatus, Lead
from src.runtime.rate_limiter import RateLimiter
from src.runtime.run_logger import RunLogger
from src.reddit.search import search_comments, split_leads_by_keyword_confirmation
...
from src.decision.templates import select_and_fill, fill_template_from_decision, fill_subject_from_decision
from src.decision.triage import triage_leads, full_triage_workflow
from src.shared.utils import load_config, load_templates, take_error_screenshot, BASE_DIR, log
from src.runtime.review_queue import (
    stage_action,
...
async def dispatch_approved_actions(only_account: str | None = None) -> dict:
    ...
    if not approved:
        print("No approved actions to dispatch.")
...
def main():
    ...
    parser.add_argument("--dispatch-approved", action="store_true",
                        help="Dispatch approved staged actions")

    args = parser.parse_args()

    if args.migrate:
        from src.runtime.state import migrate_from_old_format
        migrate_from_old_format()
        return

    if args.stats:
        from src.runtime.state import print_state_summary
        print_state_summary()
        return
