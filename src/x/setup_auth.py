...
def _ensure_supported_python(argv):
    ...
    for candidate in ("python3.12", "python3.11", "python3.10"):
        ...
        if path:
            ...
            raise SystemExit(subprocess.call([path, "-m", "src.x.setup_auth", *argv]))
...
def _ensure_venv(argv):
    ...
    requirements_path = repo_root / "requirements" / "x.txt"
    ...
    raise SystemExit(subprocess.call([str(venv_python), "-m", "src.x.setup_auth", *rerun_args], env=env))


def _install_requirements():
    requirements_path = Path(__file__).resolve().parents[2] / "requirements" / "x.txt"
...
async def main():
    parser = argparse.ArgumentParser(description="Onboard an X account and save fresh local cookies.")
    ...
    try:
        ...
        from src.x.shared.utils import load_config
    ...
    if not accounts:
        raise SystemExit("No X accounts configured in config.json")
    ...
    print(f"[AUTH] X onboarding completed for {account.get('username', '<env account>')}")
    await browser.stop()
