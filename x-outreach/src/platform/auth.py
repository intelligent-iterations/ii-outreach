...
from src.shared.utils import BASE_DIR, human_type, random_delay, take_error_screenshot, log
...
def _resolve_account(account):
    ...
    cookies_rel = os.getenv("X_COOKIES_PATH") or account.get(
        "cookies_path",
        f"data/cookies_{username}.json" if username else "data/cookies_x.json",
    )
...
async def login(config, account, headless=False):
    ...
    if not account["username"] or not account["password"]:
        raise RuntimeError(
            "X credentials missing. Set X_USERNAME/X_PASSWORD in x-outreach/.env "
            "or provide username/password in x-outreach/config.json before onboarding."
        )
    ...
    try:
        ...
        log.success(f"Cookies saved to {cookies_path}")
    except Exception as e:
